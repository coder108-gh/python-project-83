...
def url_info(id: int):
    ...
    if result["state"] == OK:
        return render_template(
            "url_info.html", data=result["data"], tests=result["checks"]
        )
    if result["state"] == DATA_NOT_FOUND:
        return "data not found"
...
# обработчики ошибок: страница и все такое (error.html)

#  строковые сообщения хорошо бы как по-другому передавать. не из repo...

# Задеплойте результат


# во флеше --- "Страница успешно проверена"
